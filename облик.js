...
/* ТЕМЫ-НАЧАЛО — не править руками: список пишет `сборка_каркаса.py`.
   От этих двух списков зависит всё, что читатель видит вокруг текста:
   хлебные крошки, строка со словами и разделами, оглавление, боковая
   панель, переходы на соседние темы. Главы, которой тут нет, для
   `облик.js` не существует — она открывается, но выглядит голой.
   Собирать обходом папки нельзя: при `file://` браузер каталог не
   читает, поэтому список именно вписывается в файл при сборке. */
const ПРЕДМЕТЫ_ТЕМ = {
  ...
  "Гинекология": {
    ...
    темы: ["anatomiya-zhpo.html", "reproduktivnaya-sistema.html", "metody-obsledovaniya.html", "instrumentalnye-metody.html", "vzomt.html"],
  },
};

const ГЛАВЫ_САЙТА = [
  {а: "pochka.html", т: "Почка", к: true},
  {а: "serdce.html", т: "Сердце", к: true},
  {а: "dyhanie.html", т: "Дыхание", к: true},
  {а: "vozbudimost.html", т: "Возбудимые ткани", к: true},
  {а: "gemostaz.html", т: "Гемостаз", к: true},
  {а: "abscess-legkogo.html", т: "Абсцесс и гангрена лёгкого", к: true},
...
(function обвязка(){
  ...
  /* Признак берётся ИЗ СПИСКА, куда его кладёт сборка: она видит файлы
     на диске, а страница — нет. Прежде тут стояло «глава есть в списке»,
     и ссылка «Краткое содержание» вела в 404 у тем без краткой версии. */
  const своя = ГЛАВЫ_САЙТА.find(г => г.а === ключГлавы);
  const естьКраткая = этоКраткая || (своя ? своя.к !== false : false);
